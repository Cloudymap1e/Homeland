...
- Runtime markup + control surface shell: `web/index.html`.
- Runtime visual styling/layout contract: `web/styles.css`.
...
- Temporary tunnel smoke route to local host: `npm run tunnel:quick`.
- Fixed tunnel route script: `npm run tunnel:run`.
...
- Runtime validation should prefer GS75-hosted/deployed target checks when practical; keep local run use minimal.
...
- If `README.md` planning sections conflict with live JS runtime files, treat `web/src/*` + this file as authoritative and patch README in the same change stream.
...
- For docs-only architecture sync runs:
  - scan recent commits for newly introduced scripts/endpoints/contracts,
  - ensure commands listed here exist in `package.json`,
  - ensure persistence, map-count, and state-machine claims match current code.
