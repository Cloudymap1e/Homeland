...
async function runPassCriteriaStandard({
  ...
  for (const mapId of orderedMaps) {
    ...
    rows.push({
      ...
      retentionAvgCarryCoins: retention.reachedTarget ? Math.max(0, retention.avgRetainedCoins) : 0,
      retentionAvgBudgetCoins: mapCfg.startingCoins + (retention.reachedTarget ? Math.max(0, retention.avgRetainedCoins) : 0),
...
function printPassCriteriaStandard(rows) {
  ...
  for (const row of rows) {
    const payload = {
      ...
      retainedCoinsAvg: Number(row.retentionAvgCarryCoins.toFixed(2)),
      retainedCoinsBudgetAvg: Number(row.retentionAvgBudgetCoins.toFixed(2)),
